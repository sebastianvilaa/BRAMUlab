...
(function (global) {
  ...
  /** V02.1 (§17, portado desde bramulab) — ¿un set determinado fue "ajustado" o "amplio"?
   *  Únicamente a partir del margen real de games (nunca inventa desarrollo punto a punto, que
   *  la carga manual no conoce) — margen <= 2 se lee como ajustado en cualquier formato (6-4,
   *  7-5, 7-6); margen >= 3 como amplio (6-3 en adelante). Umbral fijo y simple a propósito: con
   *  solo el resultado final por set disponible, no hay forma más precisa de estimarlo. */
  function isSetMarginClose(s) { return Math.abs(s.gamesA - s.gamesB) <= 2; }

  /** V02.1 (§17, portado desde bramulab) — corrige un caso real detectado: un partido a 3 sets
   *  que NO es remontada (el ganador se llevó el set 1) caía siempre en la rama "parejo" sin
   *  importar cuán lopsided hubiera sido cada set — `6-1 · 1-6 · 6-0` (dominio alternado con
   *  cierre contundente) se narraba como "desarrollo parejo", que el resultado disponible
   *  contradice directamente. Ahora se clasifica según el margen REAL de los sets 1 y 2 (nunca
   *  del resultado final "ganó/perdió" solo): ambos amplios → dominio alternado; ambos ajustados
   *  → parejo (con énfasis extra si los 3 sets fueron ajustados); mixto → competitivo, sin
   *  forzar ninguna de las dos etiquetas — el cierre del set decisivo (ver más abajo) ya aporta
   *  esa precisión. */
  ...
  function generateManualIntelligence(stats, matchCtx, sets, winnerTeam, finishInfo) {
    ...
    if (stats.straightSets) {
      // V02.1 (§17, portado desde bramulab) — "sets corridos" no implica por sí solo dominio: un
      // 7-6 · 6-4 es tan "en sets corridos" como un 6-0 · 6-0, pero solo el segundo es una
      // victoria dominante.
    ...
    if (stats.deciderSetIndex !== null && !stats.straightSets) {
      ...
      // V02.1 (§17, portado desde bramulab) — mismo criterio de margen que arriba; antes solo se
      // narraba el caso ajustado, nunca el opuesto (set decisivo amplio).
