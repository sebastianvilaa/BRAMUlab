# Nivel BRAMU V1.5 — Handoff para desarrollo

**Estado:** motor de partidos V1.0 conservado; estimador inicial V1.1 cerrado para implementar.  
**Versión del motor:** `nivel_bramu_v1_0`.  
**Versión del cuestionario:** `nivel_inicial_v1_1`.  
**Objetivo actual:** reemplazar el estimador inicial implementado en V04.3–V04.5 sin reabrir ni modificar el motor de partidos.
...
1. `docs/BRAMUlab/Nivel_BRAMU_Formula_V1.5.md`: fuente normativa vigente. Conserva el motor V1.0 y reemplaza la estimación inicial de V1.4.
...
`Nivel_BRAMU_Formula_V1.4.md` queda preservado como antecedente histórico y no debe utilizarse para implementar el cuestionario. El motor de partidos ya implementado desde esa versión permanece vigente sin cambios.
...
- pregunta final de categoría como referencia local blanda;
- ajuste automático por categoría de hasta ±0,5, sin stepper manual;
- confianza de origen variable y detección simple de incoherencias;
...
- las cinco anclas compartidas por camino rápido y autoevaluación completa;
- el cuestionario completo V1.1 con antigüedad y frecuencia como contexto, no como capacidad;
- cálculo de `nivel_base` mediante autoevaluación, red, paredes y modificador mínimo de entrenamiento;
- pregunta competitiva referida al rendimiento dentro de la categoría habitual;
- pregunta final de categoría y mapa contextual versionado;
- ajuste automático de categoría de hasta ±0,5;
- eliminación del stepper libre de ajuste;
- confianza de origen `0,12`/`0,15`/`0,18` y límite `0,10` ante incoherencia confirmada;
- detección de brecha de coherencia y acción **Revisar respuestas**;
- guardado de respuestas como origen del nivel, nunca como hechos deportivos observados;
- guardado de `questionnaire_version = nivel_inicial_v1_1` y de todos los componentes auditables;
...
- perfiles reales V1.1: Esteban 6,1; Seba 5,7; Lucho 4,8; Agustín 2,1, con tolerancia interna ±0,01;
- camino rápido con categoría para esos perfiles: 5,9; 5,5; 4,5 y 2,0;
- perfiles contradictorios que activan revisión y limitan confianza sin alterar arbitrariamente `mu`;
- categoría ausente o contexto no soportado con ajuste exactamente cero;
...
- No reutilizar los pesos 30/15/10/10/15/10/10 ni `1 + 7,5 × Q` para altas nuevas.
- No aplicar el mapa argentino a un contexto territorial, circuito o división no validado.
- No conservar el stepper de ±0,5 como un segundo ajuste adicional.
...
## 7. Próxima tarea para desarrollo

V04.1–V04.5 ya implementaron y probaron el motor, la elegibilidad y el primer recorrido visual. La próxima ronda no debe repetir las Etapas A–B ni rediseñar Nivel BRAMU.

La tarea es reemplazar de manera acotada el estimador inicial de V04.3–V04.5:

- actualizar `level-calibration.js` con las reglas de `Nivel_BRAMU_Formula_V1.5.md` §3;
- adaptar el onboarding para la pregunta final de categoría;
- retirar el stepper manual;
- versionar y persistir el nuevo origen del nivel;
- reemplazar únicamente los fixtures del cuestionario anterior y agregar los perfiles V1.1;
- mantener intacto `level.js`, sus parámetros y todos los fixtures del motor de partidos;
- conservar el feature flag hasta aprobar pruebas automáticas y recorrido visual.

El desarrollo debe comenzar con un diagnóstico breve de archivos afectados y después puede implementar este bloque en una única ronda acotada.
...
**Motor de partidos:** cerrado y sin cambios (`nivel_bramu_v1_0`).  
**Estimador inicial:** cerrado para implementar (`nivel_inicial_v1_1`).  
**Lista para desarrollo:** sí.  
**Lista para desplegar a producción:** todavía no; primero debe reemplazarse el cuestionario implementado, superar el arnés y aprobarse visualmente.  
**Próxima acción:** implementar el bloque acotado de cuestionario V1.1 sin avanzar a otras funciones.
