...
`delta_i = K_i × (resultado − expectativa) × margen × formato × repeticion × disponibilidad × factor_oponente`
...
`calidad_evidencia = formato × repeticion × disponibilidad × (0,55 + 0,45 × confianza_pareja_rival)`
...
`6,00 → 6,01 → 6,02 → 6,03 → 6,03 → 6,04 → … → 6,06`
...
- grupo fijo de ocho jugadores entre 4,0 y 5,0;
...
| Efectividad anual | Récord | Nivel inicial | Nivel a 25 partidos | Nivel a 100 | Nivel a 200 |
|---:|---:|---:|---:|---:|---:|
| 60% | 120–80 | 4,20 | 4,51 | 4,75 | 4,90 |
| 65% | 130–70 | 4,20 | 4,62 | 4,92 | 5,13 |
| 70% | 140–60 | 4,20 | 4,71 | 5,11 | 5,38 |

En la versión anterior de parámetros, 65% llevaba aproximadamente a 5,64 y 70% a 6,05. Esa deriva se consideró excesiva para una burbuja cerrada y motivó reducir `K` en jugadores confiables y cambiar el divisor de expectativa de 2,5 a 1,5.

La lectura correcta no es que ganar 70% “no importe”: un 70% sostenido durante 200 encuentros demuestra una ventaja clara dentro de ese grupo. Sin embargo, pasar de 4,2 a aproximadamente 5,4 resulta mucho más defendible que saltar a 6,0 sin haberse probado frente a otro ecosistema. Si parte de los partidos tiene solamente tres o dos niveles conocidos, los factores 0,80 y 0,60 reducen todavía más el desplazamiento.
