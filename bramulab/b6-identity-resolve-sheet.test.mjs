...
// `#identity-resolve-scrim` (18_Handoff_Revalidacion_Hotfix_Resumen_Work.md, Caso 2 BLOQUEADO).
...
// ESTÁTICA sobre el código fuente, no una ejecución: extrae el cuerpo exacto de
// `openIdentityResolveSheet`/`closeIdentityResolveSheet` y confirma que siguen el mismo patrón
// `hidden` + `is-open` que el resto de los ~11 sheets de `.sheet-scrim` (ver
// `load-player-sheet-scrim` en el mismo archivo) — el bug real era que el scrim solo alternaba
// `hidden`, sin agregar/quitar `is-open`, así que quedaba en el DOM con `opacity:0` y la hoja
// desplazada fuera de pantalla (`transform: translateY(100%)`, CSS en styles.css).
