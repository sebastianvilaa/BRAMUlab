-- BRAMUlab — Bloque 6: reaplicación explícita de create_or_attach_match para C-03/C-04.
--
-- Staging ya tiene aplicada la migración histórica de Bloque 5 que definió esta función.
-- Durante Bloque 6 se corrigieron dos comportamientos que dependen de su implementación:
--   C-03: no crear duplicados cuando un match tiene un slot sin identificar por incidencia;
--   C-04: al adjuntar a un match existente, preservar la orientación A/B almacenada.
--
-- Esta migración NO reabre Bloque 5: reaplica exactamente la definición vigente de
-- create_or_attach_match para que esos cambios lleguen al esquema real, sin editar la historia
-- de migraciones ya ejecutada en Staging.
--
-- BRAMUlab — Backend Bloque 5 hotfix: no duplicar conformidad de pareja
